...
#endif
{
  ...
  // -------- SCARA/COREXY BUG FIX START --------
  // This section has been modified to correctly handle non-cartesian kinematics.
  
  // A. Get current cartesian position from planner's step position.
  float current_position[N_AXIS];
  system_convert_array_steps_to_mpos(current_position, pl.position);

  // B. Calculate cartesian distance and unit vector.
  float delta_mm, unit_vec[N_AXIS];
  uint8_t idx;
  for (idx=0; idx<N_AXIS; idx++) {
      delta_mm = target[idx] - current_position[idx];
      unit_vec[idx] = delta_mm; // Store numerator. Denominator computed later.
      block->millimeters += delta_mm*delta_mm;
  }
  block->millimeters = sqrt(block->millimeters);

  // C. Calculate motor steps and direction.
  ...
  for (idx=0; idx<N_AXIS; idx++) {
      ...
      if (target_steps[idx] < pl.position[idx]) {
          block->direction_bits |= get_direction_pin_mask(idx);
      }
  }
  
  // D. Bail if this is a zero-length block.
  if (block->millimeters == 0) { return; } 

  // E. Normalize unit vector.
  ...
  // Calculate the block maximum feed rate and acceleration scaled down such that no individual
  // axes maximum values are exceeded with respect to the line direction. 
