...
void system_convert_mpos_to_array_steps(int32_t *target_steps, float* position)
{
  ...
  //   return target_scara;
  // }
  float theta = 2 * quick_atan2(sqrt(r_squared - sm), sqrt(s - r_squared));
  float angle = quick_atan2(y, x) + quick_atan2(2 * l1 * l2 * sin(theta),
    l1_squared + r_squared - l2_squared);
  target_steps[U_MOTOR] = lround(settings.steps_per_mm[U_MOTOR] * angle);
  target_steps[L_MOTOR] = lround(settings.steps_per_mm[L_MOTOR] * (theta - M_PI + angle));
  for (idx=0; idx<N_AXIS; idx++) {
